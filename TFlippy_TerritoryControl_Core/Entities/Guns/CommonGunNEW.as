		...
		for (int i = 0; i < count; i++)
		{
			...
			if(isServer()) 
			{
				...
				if(mapHit)
				{
					...
					if((map.isTileWood(tile) || damage >= 0.60f) && !map.isTileBedrock(tile) && XORRandom(4) == 0)
					{
						map.server_DestroyTile(hitPos, damage * 0.15f);
					}
				}
			}
		}
...
void onInit(CSprite@ this)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		CSpriteLayer@ tracer=this.addSpriteLayer("tracer" + i, (this.getBlob().exists("gun_tracerName") ? this.getBlob().get_string("gun_tracerName") : "GatlingGun_Tracer.png"),32,2,this.getBlob().getTeamNum(),0);
