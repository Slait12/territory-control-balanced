...
void onTick(CBrain@ this)
{
	...
	if (target !is null && target !is blob && (target.hasTag("player") || target.hasTag("weapon") || target.hasTag("combat chicken")) && !target.hasTag("dead"))
	{
		// print("" + target.getConfig());
		...
		blob.setAimPos(target.getPosition()-Vec2f(0,(blob.getPosition()-target.getPosition()).getLength()/8));
...
CBlob@ FindTarget(CBrain@ this, f32 maxDistance)
{
	...
	for (int i = 0; i < blobs.length; i++)
	{
		...
		if (d < distance && b.getTeamNum() != myTeam && !b.hasTag("dead") && !b.hasTag("invincible") && !b.hasTag("no targeting"))
		{
			distance = d;
...
void Move(CBrain@ this, CBlob@ blob, Vec2f pos)
{
	...
	blob.AddForce(Vec2f(dir.x > 0 ? -250.0f : 250.0f, 0));
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	if (isClient())
	{
		if (getGameTime() > this.get_u32("next sound") - 50)
		{
			...
			this.set_u32("next sound", getGameTime() + 30);
		}
	}
