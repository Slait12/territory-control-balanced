...
const Vec2f arm_offset = Vec2f(2, 1);
...
void onInit(CBlob@ this)
{
	...
	Vehicle_SetupWeapon(this, v,
	                    ...
	                    "mat_highcalshell", // bullet ammo config name
	                    "sfcannonshell", // bullet config name
	                    "SFCannon_Shot", // fire sound
	...
	this.set_string("ammoIcon", "mat_highcalshell");
    ...
    Vehicle_addWheel(this, v, "SFCannonWheel.png", 12, 12, 0, Vec2f(-25.0f, 15.0f));
	Vehicle_addWheel(this, v, "SFCannonWheel.png", 12, 12, 0, Vec2f(-14.0f, 15.0f));
	Vehicle_addWheel(this, v, "SFCannonWheel.png", 12, 12, 0, Vec2f(-3.0f, 15.0f));
	Vehicle_addWheel(this, v, "SFCannonWheel.png", 12, 12, 0, Vec2f( 8.0f, 15.0f));
	Vehicle_addWheel(this, v, "SFCannonWheel.png", 12, 12, 0, Vec2f( 19.0f, 15.0f));
	Vehicle_addWheel(this, v, "SFCannonWheel.png", 12, 12, 0, Vec2f( 30.0f, 15.0f));
	...
	CSpriteLayer@ arm = sprite.addSpriteLayer("arm", "SFCannon_Barrel.png", 115, 24);
	if (arm !is null)
	{
	...
	this.set_string("autograb blob", "mat_highcalshell");

	if (isServer())
	{
		CBlob@ ammo = server_CreateBlob("mat_highcalshell");
		if (ammo !is null)
		{
...
void onTick(CBlob@ this)
{
	if (this.hasAttached() || this.getTickSinceCreated() < 30) //driver, seat or gunner, or just created
	{
		...
		if (arm !is null)
		{
			...
			arm.RotateBy(rotation, Vec2f(facing_left ? -4.0f : 4.0f, 0.0f)); //gun offset
		}
...
void Vehicle_onFire(CBlob@ this, VehicleInfo@ v, CBlob@ bullet, const u8 _unused)
{
	if (bullet !is null)
	{
		...
		Vec2f offset = Vec2f((this.isFacingLeft() ? -1 : 1) * 60, -14); //bullet offset
...
void onDie(CBlob@ this) 
{
	if (isServer())
	{
		...
		CBlob@ wreck = server_CreateBlobNoInit("sfcannonwreck");
