...
const Vec2f arm_offset = Vec2f(-5, -3);
...
void onInit(CBlob@ this)
{
	...
	Vehicle_SetupWeapon(this, v,
	                    ...
	                    "mine", // bullet ammo config name
	                    "mine", // bullet config name
	...
	CSpriteLayer@ arm = sprite.addSpriteLayer("arm", "Minethrower_Cannon.png", 18, 9);
	if (arm !is null)
	{
	...
	this.set_string("autograb blob", "mine");
	this.set_string("ammoIcon", "mine");
	...
	if (isServer())
	{
		CBlob@ ammo = server_CreateBlob("mine");
		if (ammo !is null)
		{
...
void Vehicle_onFire(CBlob@ this, VehicleInfo@ v, CBlob@ bullet, const u8 _unused)
{
	if (bullet !is null)
	{
		...
		Vec2f vel = Vec2f(28.0f * (this.isFacingLeft() ? -1 : 1), 0.0f).RotateBy(angle);
