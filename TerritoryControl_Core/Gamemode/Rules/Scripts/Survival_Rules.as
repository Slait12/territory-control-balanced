			...
			if (blob is null && player.get_u32("respawn time") <= gametime)
			{
				...
				if (isNeutral)
				{
					...
					if (default_respawn)
					{
						...
						else if (this.hasTag("tournament"))
						{
							doDefaultSpawn(player, "grandpa", 100, false);
						}
						else
						{
							if (XORRandom(100) < 33)
...
bool doDefaultSpawn(CPlayer@ player, string blobType, u8 team, bool ignoreDisabledSpawns)
{
	...
	else if (getRules().hasTag("tournament"))
	{
		CMap@ map = getMap();
		if (map is null) return false;
		CBlob@ new_blob = server_CreateBlobNoInit("grandpa");
		new_blob.server_setTeamNum(100);
		new_blob.setPosition(Vec2f(map.tilemapwidth/2*8, 32));
		new_blob.Tag("nosound");
		new_blob.Init();
		new_blob.server_SetPlayer(player);
		return true;
	}
