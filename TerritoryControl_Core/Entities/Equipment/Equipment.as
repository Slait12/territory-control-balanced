	...
	if (cmd == this.getCommandID("equip_head") || cmd == this.getCommandID("equip_torso") || cmd == this.getCommandID("equip2_torso") || cmd == this.getCommandID("equip_boots"))
	{
		...
		if (item !is null)
		{
			...
			if (getEquipmentType(item) == "head" && cmd == this.getCommandID("equip_head"))
			{
				...
				if (eqName == "militaryhelmet" || eqName == "compositehelmet" || eqName == "lighthelmet" || eqName == "bucket" || eqName == "pumpkin" || 
			...
			else if (getEquipmentType(item) == "torso" && cmd == this.getCommandID("equip_torso"))
			{
				...
				if (eqName == "bulletproofvest" || eqName == "compositevest" || eqName == "lightvest" || eqName == "keg") caller.set_f32(eqName+"_health", item.get_f32("health"));
				item.server_Die();
			}
			else if (getEquipmentType(item) == "torso" && cmd == this.getCommandID("equip2_torso"))
			{
				if (eqName == "bulletproofvest" || eqName == "compositevest")
				{
					return;
				}
			...
			else if (getEquipmentType(item) == "boots" && cmd == this.getCommandID("equip_boots"))
			{
				...
				if (eqName == "combatboots" || eqName == "compositeboots") caller.set_f32(eqName+"_health", item.get_f32("health"));
				item.server_Die();
			}
...
void removeHead(CBlob@ playerblob, string headname)
{
	...
	if (isServer())
	{
		...
		else if (headname == "compositehelmet" || headname == "lighthelmet" || headname == "bucket" || headname == "pumpkin" || headname == "scubagear" || headname == "minershelmet")
		{
			oldeq.set_f32("health", playerblob.get_f32(headname+"_health"));
		}
...
void removeTorso(CBlob@ playerblob, string torsoname)		//Same stuff with removing again.
{
	...
	if (isServer())
	{
		...
		if (torsoname == "bulletproofvest" || torsoname == "compositevest" || torsoname == "lightvest" || torsoname == "keg") 
...
void removeBoots(CBlob@ playerblob, string bootsname)		//I think you should already get how this works.
{
	...
	if (isServer())
	{
		...
		if (bootsname == "combatboots" || bootsname == "compositeboots") oldeq.set_f32("health", playerblob.get_f32(bootsname+"_health"));
		playerblob.server_PutInInventory(oldeq);		
	}
...
void onDie(CBlob@ this)
{
    if (isServer())
	{
		...
		if (headname != "")
		{
			if (headname == "compositehelmet" || headname == "lighthelmet" || headname == "bucket" || headname == "pumpkin" || headname == "scubagear" || headname == "minershelmet" || headname == "nvd")
			{
		...
		if (torsoname != "")
		{
			if (torsoname == "bulletproofvest" || torsoname == "compositevest" || torsoname == "lightvest")
			{
		...
		if (torso2name != "")
		{
			if (torso2name == "bulletproofvest" || torso2name == "compositevest" || torso2name == "lightvest")
			{
		...
		if (bootsname != "")
		{
			if (bootsname == "combatboots" || bootsname == "compositeboots")
			{
